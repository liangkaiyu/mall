...
if(!empty($_POST['name'])){
    ...
    //入库处理
    $sql="insert `liangkaiyu_goods`(`name`,`price`,`des`,`content`,`pic`,`user_id`,`create_time`,`update_time`,`view`) values ('{$name}','{$price}','{$des}','{$content}','{$pic}','{$userId}','{$now}','{$now}',0)";

    //echo $sql;die;
    //插入数据
    if($query=mysqli_query($con,$sql)){
        msg(1,'商品发布成功','index.php');
    }else{
    ...
    <div class="addwrap">
        <div class="addl fl">
            ...
            <form name="publish-form" id="publish-form" action="publish.php" method="post"
                ...
                <div class="additem">
                    <!-- 使用accept html5属性 声明仅接受png gif jpeg格式的文件,简单过滤-->
